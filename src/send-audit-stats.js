...
const sendAuditStats = (req, res, next) => {
  ...
  // send number of calls to this route (with response status code) with 1% sampling
  const routeName = req.route ? 'route.' + req.route.name : 'invalid_route';
  stats.increment(routeName + '.status.' + res.statusCode, 1, 0.01);

  // send error statuses (with response status code) with 1% sampling
  if (res._body && res._body.restCode) {
    stats.increment(routeName + '.code.' + cleanupStatsKey(res._body.restCode), 1, 0.1);
  }

  // send timings with 0.1% sampling
  (req.timers || []).forEach((timer) => {
    const t = timer.time;
    const n = cleanupStatsKey(timer.name);
    stats.timing(routeName + '.timers.' + n, 1000000000 * t[0] + t[1], 0.001);
  });
